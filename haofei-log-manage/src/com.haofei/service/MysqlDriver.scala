...
object MysqlDriver {
  def main(args: Array[String]): Unit = {
    ...
    sxgfSource.foreachRDD { rdd =>
      val sqldata = RddUtil.rddToSql(rdd, mapValue(0))
      MysqlUtil.saveToMysql(sqldata.collect, dsValue(0))
      println(sdf.format(System.currentTimeMillis) + "|sxgf_tslog|" + sqldata.count)
    }

    // 手心渠道业务逻辑处理
    sxqdSource.foreachRDD { rdd =>
      val sqldata = RddUtil.rddToSql(rdd, mapValue(1))
      MysqlUtil.saveToMysql(sqldata.collect, dsValue(1))
      println(sdf.format(System.currentTimeMillis) + "|sxqd_tslog|" + sqldata.count)
    }

    // 天天官方业务逻辑处理
    ttgfSource.foreachRDD { rdd =>
      val sqldata = RddUtil.rddToSql(rdd, mapValue(2))
      MysqlUtil.saveToMysql(sqldata.collect, dsValue(2))
      println(sdf.format(System.currentTimeMillis) + "|ttgf_tslog|" + sqldata.count)
    }

    // 天天渠道业务逻辑处理
    ttqdSource.foreachRDD { rdd =>
      val sqldata = RddUtil.rddToSql(rdd, mapValue(3))
      MysqlUtil.saveToMysql(sqldata.collect, dsValue(3))
